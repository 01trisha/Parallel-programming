...
// координаты узла с произвольным индексом
    ...
    for (int j = 1; j < N_Y - 1; ++j)
    {
      for (int k = 1; k < N_Z - 1; ++k)
      {
        ...
        f_i = (prevFunc[GetIndex(i + 1, j, k)] +
              ...
              H_X_2;
        f_j = (prevFunc[GetIndex(i, j + 1, k)] +
               prevFunc[GetIndex(i, j - 1, k)]) /
              H_Y_2;
        f_k = (prevFunc[GetIndex(i, j, k + 1)] +
               prevFunc[GetIndex(i, j, k - 1)]) /
              H_Z_2;

        currFunc[GetIndex(i, j, k)] =
            (f_i + f_j + f_k - Rho(GetX(offset + i), GetY(j), GetZ(k))) /
            (2 / H_X_2 + 2 / H_Y_2 + 2 / H_Z_2 + A);

        tmpMaxDiff =
            fabs(currFunc[GetIndex(i, j, k)] - prevFunc[GetIndex(i, j, k)]);
    ...
    for (int k = 1; k < N_Z - 1; ++k)
    {
      if (rankProc != 0)
      {
        ...
        f_j = (prevFunc[GetIndex(0, j + 1, k)] +
               prevFunc[GetIndex(0, j - 1, k)]) /
              H_Y_2;
        f_k = (prevFunc[GetIndex(0, j, k + 1)] +
               prevFunc[GetIndex(0, j, k - 1)]) /
              H_Z_2;

        currFunc[GetIndex(0, j, k)] =
            (f_i + f_j + f_k - Rho(GetX(offset), GetY(j), GetZ(k))) /
            (2 / H_X_2 + 2 / H_Y_2 + 2 / H_Z_2 + A);

        tmpMaxDiff =
            fabs(currFunc[GetIndex(0, j, k)] - prevFunc[GetIndex(0, j, k)]);
        if (tmpMaxDiff > maxDiff)
          maxDiff = tmpMaxDiff;
      }
      ...
      if (rankProc != countProc - 1)
      {
        f_i = (prevFunc[GetIndex(layerHeight - 2, j, k)] +
              ...
              H_X_2;
        f_j = (prevFunc[GetIndex(layerHeight - 1, j + 1, k)] +
               prevFunc[GetIndex(layerHeight - 1, j - 1, k)]) /
              H_Y_2;
        f_k = (prevFunc[GetIndex(layerHeight - 1, j, k + 1)] +
               prevFunc[GetIndex(layerHeight - 1, j, k - 1)]) /
              H_Z_2;

        currFunc[GetIndex(layerHeight - 1, j, k)] =
            (f_i + f_j + f_k -
             Rho(GetX(offset + layerHeight - 1), GetY(j), GetZ(k))) /
            (2 / H_X_2 + 2 / H_Y_2 + 2 / H_Z_2 + A);

        tmpMaxDiff = fabs(currFunc[GetIndex(layerHeight - 1, j, k)] -
                          prevFunc[GetIndex(layerHeight - 1, j, k)]);
        if (tmpMaxDiff > maxDiff)
          maxDiff = tmpMaxDiff;
      }
    }
...
int main(int argc, char **argv)
{
  ...
  upLimitLayer =
      malloc(sizeof(double) * N_Y *
             N_Z); // нижний слой пред процесса для расчета верхней границы
  downLimitLayer =
      malloc(sizeof(double) * N_Y *
  ...
  do
  {
    ...
    if (rankProc != countProc - 1)
    {
      double *prevDownLimit =
          ...
          (layerHeights[rankProc] - 1) * N_Y *
              N_Z; // нижний слой тек процесса отправляемый след процессу
      MPI_Isend(prevDownLimit, N_Y * N_Z, MPI_DOUBLE, rankProc + 1, rankProc,
                MPI_COMM_WORLD, &sendDownReq);
      MPI_Irecv(downLimitLayer, N_Y * N_Z, MPI_DOUBLE, rankProc + 1,
                rankProc + 1, MPI_COMM_WORLD, &recvDownReq);
    }
    ...
    procMaxDiff = CalcLimit(prevFunc, currFunc, upLimitLayer, downLimitLayer,
                            layerHeights[rankProc], offsets[rankProc], rankProc,
                            countProc);
    procMaxDiff = fmax(tmpMaxDiff, procMaxDiff);

    MPI_Allreduce(&procMaxDiff, &maxDiff, 1, MPI_DOUBLE, MPI_MAX,
                  MPI_COMM_WORLD);
