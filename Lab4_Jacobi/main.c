...
void InitLayers(double *prevFunc, double *currFunc, int layerHeight, int offset)
{
    for (int i = 0; i < layerHeight; ++i)
    {
        for (int j = 0; j < N_Y; j++)
        {
            for (int k = 0; k < N_Z; k++)
            {
                bool isBorder = (offset + i == 0) || (j == 0) || (k == 0) || (offset + i == N_X - 1) || (j == N_Y - 1) || (k == N_Z - 1);
                if (isBorder)
                {
...
int main(int argc, char **argv)
{
    ...
    do
    {
        ...
        // Calculate border
        procMaxDiff = CalcLimit(prevFunc, currFunc, upLimitLayer, downLimitLayer, layerHeights[rankProc], offsets[rankProc], rankProc, countProc);
